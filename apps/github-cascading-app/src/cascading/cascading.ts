...
export abstract class Cascading {
   ...
   */
  protected abstract createPullRequest(cascadingBranch: string, targetBranch: string, body: string, title: string): Promise<void>;
  /**
...
---`;
  ...
  protected async createPullRequestWithMessage(cascadingBranch: string, currentBranch: string, targetBranch: string, config: CascadingConfiguration, shouldAddUpdateMessage = false) {
    ...
    const title = `[cascading] from ${currentBranch} to ${targetBranch}`;
    ...
    await this.createPullRequest(cascadingBranch, targetBranch, message, title);
  }
