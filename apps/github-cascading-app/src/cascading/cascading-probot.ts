...
export class CascadingProbot extends Cascading {
  ...
  protected async loadConfiguration(currentBranch?: string) {
    ...
    if (configFileValidResponses.length > 1) {
      this.logger.warn(`Several configuration files have been found (${configFileValidResponses.map((c) => Array.isArray(c!.data) && c!.data[0].path).join(', ')}). The files will be ignored.`);
  ...
  /** @inheritdoc */
  protected async createPullRequest(cascadingBranch: string, targetBranch: string, body: string, title: string) {
    // TODO: add auto_merge when allow by the RestAPI (cf: https://github.com/orgs/community/discussions/24719)
    await this.options.octokit.pulls.create({
      ...this.options.repo,
