...
export const prepareProject = (options: NgAddSchematicsSchema, rootFolder: string) => async (tree: Tree, context: SchematicContext) => {
  ...
  return () => chain([
    ...
    // task that should run after the schematics should be after the ng-add task as they will wait for the package installation before runing the other dependencies
